...
int mbedtls_x509_crt_parse(mbedtls_x509_crt *chain,
                           const unsigned char *buf,
                           size_t buflen, int format)
{
#if defined(MBEDTLS_PEM_PARSE_C)
    int success = 0, first_error = 0, total_failed = 0;
    int buf_format = format;
#endif
...
int mbedtls_x509_crt_parse_file(mbedtls_x509_crt *chain, const char *path, int format)
{
    ...
    ret = mbedtls_x509_crt_parse(chain, buf, n, format);
...
int mbedtls_x509_crt_parse_path(mbedtls_x509_crt *chain, const char *path)
{
    ...
    do {
        ...
        w_ret = mbedtls_x509_crt_parse_file(chain, filename, MBEDTLS_X509_FORMAT_PEM);
        if (w_ret < 0) {
            ret++;
        } else {
            ret += w_ret;
